...
const { handleErr, sendEmail, logAdminAction } = require("../utils");
...
module.exports = {
  ...
  register: (req, res) => {
    ...
    if (!email || !username || !password)
      return handleErr(res, 422, "Please fill in all fields.");
    User.find({ $or: [{ email }, { username }] }, (err, users) => {
      if (err) return handleErr(res, 500, "error from finding", err);
      if (users.length > 0)
        return res.status(409).send({
          message: "A user already exists with either this username or email",
          users
        });
      ...
      newUser.save((err, user) => {
        ...
        User.populate(user, populateOptions, (error, moun) => {
          sendEmail.welcome(user.email).then(
            result =>
              addToList(user.email, MEMBER).then(
                () => res.status(200).send({ token, user: moun }),
                e => {
                  if (e.status === 400)
                    return res.status(200).send({ token, user: moun });
    ...
    User.findById(req.params.id)
      ...
      .then(
        user =>
          user
            ? res.json(user)
            : handleErr(res, 404, "User information could not be found."),
        err => handleErr(res, 500)
      );
  },
  forgotPass: (req, res) => {
    ...
    const emailUser = (user, token, done) => {
      sendEmail
        .forgotPassword(user.email, token)
        .then(result => done(null, result, user, token), err => done(err));
    };
    async.waterfall(
      [getToken, addToUser, emailUser],
      (err, result, user, token) => {
        if (err)
          return typeof err === "string"
            ? handleErr(res, 501, err)
            : handleErr(res, 500);
        ...
        user => {
          ...
          user.save((err, newUser) => {
            ...
            User.populate(newUser, populateOptions, (error, person) => {
              ...
              sendEmail
                .pwResetSuccess(newUser.email)
                .then(
                  emailresult => res.json(person),
                  err => handleErr(res, 500)
                );
            });
          });
  ...
  toggleActive: (req, res) => {
    User.findById(req.params.id)
      .then(user => {
        if (!user) return handleErr(res, 404, `User object not found.`);
  ...
  populatedUser: (req, res) => {
    User.findById(req.params.id)
      ...
      .exec((err, user) => {
        ...
        return user
          ? res.json(user)
          : handleErr(res, 404, "Could not locate user information");
      });
  },
  addVote: (req, res) => {
    const addVoteToUser = done => {
      User.findByIdAndUpdate(
        ...
        (err, user) => {
          if (err)
            return done({
              message: "Error updating your account with this like.",
              data: err
            });
          User.populate(user, populateOptions, (error, person) => {
            if (error)
              return done({
                message: "Please restart your browser and try again",
                data: error
              });
            done(null, person);
          });
    ...
    const addVoteToTerm = (user, done) => {
      ...
      Term.findByIdAndUpdate(
        ...
        (err, word) => {
          if (err)
            return done({
              message: "Could not add a Like to this term",
              data: err
            });
          Term.findById(word._id)
            .populate("author sentences.author")
            .exec((err2, term) => {
              if (err2)
                return done({
                  message:
                    "Term liked, but you may need to restart your browser.",
                  data: err2
                });
              done(null, term, user);
            });
    ...
    const addNotificationToAuthor = (term, user, done) => {
      ...
      const notification = {
        text: `${user.username} liked your definition for: ${titleCase(
          term.text
        )}`,
      ...
      User.findByIdAndUpdate(
        ...
        (err, author) => {
          ...
          if (err)
            return done({
              message: `Term liked successfully. But you may need to restart.`,
              data: err
            });
          const data = { term, user };
          if (!author.notificationSettings.likes)
            return done(null, author, term, user);
          sendEmail.liked(author.email, data).then(
            () => done(null, author, term, user),
            error =>
              done({
                message: `Term liked, but you may need to restart your browser.`,
                data: error
              })
          );
        }
    ...
    async.waterfall(
      [addVoteToUser, addVoteToTerm, addNotificationToAuthor],
  ...
  minusVote: (req, res) => {
    const removeFromUser = done => {
      User.findByIdAndUpdate(
        ...
        (err, person) => {
          if (err)
            return done({
              message: "Could not perform operation, try again.",
              data: err
            });
    ...
    const removeFromTerm = (user, done) => {
      Term.findByIdAndUpdate(
        ...
        (err, word) => {
          if (err)
            return done({
              message: "Server error with this operation.",
              data: err
            });
          const opts = [
            { path: "author" },
    ...
    async.waterfall([removeFromUser, removeFromTerm], (err, term, user) => {
      if (err) return handleErr(res, 503, err.message, err.data);
      res.json({ user, term });
    });
  },
  all: (req, res) => {
    User.find().exec(
      (err, users) => (err ? handleErr(res, 500) : res.json(users))
    );
  },
                ...
                user.save((err, data) => {
                  if (err)
                    return handleErr(
                      res,
                      500,
                      "User added to list, but error trying to save user as subscribed in database.",
                      err
                    );
      ...
      .then(
        user => {
          if (!user.validPassword(current))
            return handleErr(res, 401, "Incorrect password. Try again.");
          user.password = user.generateHash(password);
          const notification = {
          ...
          user.save((err, data) => {
            ...
            User.populate(data, populateOptions, (error, person) => {
              if (error)
                return handleErr(res, 503, "Please restart your browser.");
              res.json(person);
            });
          });
        },
  ...
  tgl_notification_for_sentences: (req, res) => {
    User.findById(req.params.id)
      ...
      .then(
        user => {
          user.notificationSettings.sentences = !user.notificationSettings
            .sentences;
          user.save((err, result) => {
            if (err) return handleErr(res, 500);
            User.populate(result, populateOptions, (error, person) => {
  ...
  tgl_notification_for_achievements: (req, res) => {
    User.findById(req.params.id)
      ...
      .then(
        user => {
          user.notificationSettings.achievements = !user.notificationSettings
            .achievements;
          user.save((err, result) => {
            if (err) return handleErr(res, 500);
            User.populate(result, populateOptions, (error, person) => {
      ...
      (err, user) => {
        if (err)
          return handleErr(
            res,
            500,
            "Something went wrong setting all notifications off.",
            err
          );
        User.populate(user, populateOptions, (error, person) => {
          if (error) return handleErr(res, 503, "Please restart your browser");
  ...
  saveTerm: (req, res) => {
    ...
    if (!term)
      return handleErr(
        res,
        400,
        "You may need to restart your browser before you can save this term."
      );
    User.findById(req.params.id)
      .exec()
      .then(
        user => {
          for (let i = 0; i < user.savedTerms.length; i++) {
            if (user.savedTerms[i].term == term)
              return handleErr(res, 400, "You already saved this term.");
          }
  ...
  sendCustomEmail: (req, res) => {
    console.log(`Sending email: `, req.body.user, req.body.email);
    const { user, email } = req.body;
    sendEmail
      .custom(user.email, email.subject, email.body)
      .then(result => res.json({ message: "email sent" }))
      .catch(e => handleErr(res, 500));
  },
  makeSuper: (req, res) => {
    User.findById(req.params.id)
      ...
      .then(
        person => {
          if (!person)
            return handleErr(res, 404, "That user account cannot be found.");
          if (person.username !== "clervius")
            return handleErr(
              res,
              401,
              "Your account must be made into an Admin by an existing admin."
            );
          person.role = "super";
