...
const Log = require("./adminlog-model");
...
const sendToUser = (type, to, subject, body) => {
  ...
  return new Promise((resolve, reject) => {
    const msg = {
      ...
      html: type.html,
      templateId: "d57ed644-ae99-4e6e-96ea-4d6c34b867dc"
    };
...
module.exports = {
  handleErr: (res, status, message, data) => {
    ...
    return status === 500
      ? res.status(500).send({ message: "Server error with this operation." })
  ...
  logAdminAction: (admin, event) => {
    return new Promise((resove, reject) => {
      newItem = new Log();
      newItem.event = event;
      newItem.user = admin;
      newItem.save((err, logItem) => (err ? reject(err) : resolve(logItem)));
    });
  },
  ...
  sendEmail: {
    ...
    sentenceAdded: (to, data) =>
      sendToUser(emails.notify_sentence_added(data), to)
  }
};
